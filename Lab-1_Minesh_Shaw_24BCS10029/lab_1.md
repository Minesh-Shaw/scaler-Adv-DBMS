...
2. **Inode lookup** — each directory entry maps a filename → inode number. The kernel fetches the inode from the filesystem (ext4/btrfs/etc.) via the VFS layer.
...
```bash
# While the program is running (add a sleep), check open fds:
ls -l /proc/<PID>/fd
